...
```shell
$ wget https://cloud-images.ubuntu.com/noble/current/noble-server-cloudimg-amd64.img # x86-64
$ wget https://cloud-images.ubuntu.com/noble/current/noble-server-cloudimg-arm64.img # AArch64
$ qemu-img convert -p -f qcow2 -O raw noble-server-cloudimg-amd64.img noble-server-cloudimg-amd64.raw # x86-64
$ qemu-img convert -p -f qcow2 -O raw noble-server-cloudimg-arm64.img noble-server-cloudimg-arm64.raw # AArch64
```
...
$ ./cloud-hypervisor \
	...
	--disk path=noble-server-cloudimg-arm64.raw path=/tmp/ubuntu-cloudinit.img \
	--cmdline "console=hvc0 root=/dev/vda1 rw" \
	--cpus boot=4 \
	--memory size=1024M \
	--net "tap=,mac=,ip=,mask="
...
$ ./cloud-hypervisor \
	...
	--console off \
	--serial tty \
	--disk path=noble-server-cloudimg-arm64.raw \
...
- [Ubuntu Focal](https://cloud-images.ubuntu.com/noble/current/) (noble-server-cloudimg-{amd64,arm64}.img)
